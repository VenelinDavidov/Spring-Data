...
@DataJpaTest
public class TestLibraryMembersTable {
    ...
    @Test
    void testLibraryMembersTable() throws SQLException {
        DatabaseMetaData metaData = getDatabaseMetaData();

        List<String> expectedResult = new ArrayList<>();
        expectedResult.add("ID,BIGINT,0,19,19");
        expectedResult.add("ADDRESS,VARCHAR,1,255,255");
        expectedResult.add("FIRST_NAME,VARCHAR,0,255,255");
        expectedResult.add("LAST_NAME,VARCHAR,0,255,255");
        expectedResult.add("PHONE_NUMBER,VARCHAR,0,255,255");

        ResultSet columns = metaData.getColumns(null, "PUBLIC", "LIBRARY_MEMBERS", null);
